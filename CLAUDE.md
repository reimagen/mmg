...
- Live agent is **Mac**. Client delegation. No video on Live. Browser mic is the environment.
- P0: bank the record (`upsert`/`log`/`brief`) + killable Exa research.
- Memory is SQLite in-process (`@/lib/memory`), local-only. Never await Exa or Hermes on the live path.
