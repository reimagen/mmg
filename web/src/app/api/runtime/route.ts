...
export async function GET(request: Request) {
  ...
  return NextResponse.json({
    ...
    // Homework done before the room: names Mac expects to hear, and what it already knows about them.
    roster: (() => {
      const roster = listRoster();
      return {
        count: roster.length,
        with_context: roster.filter((e) => e.blurb).length,
        people: roster.map((e) => ({ name: e.name, org: e.org, role: e.role, blurb: e.blurb, url: e.url })),
      };
    })(),
    traces: listTraces(),
