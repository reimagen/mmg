...
case "${1:-status}" in
  up)
    ...
    up_one web      "$ROOT/web" env MEMORY_API_URL=http://127.0.0.1:7777 HERMES_ENABLED=0 BACKEND_LLM=1 BACKEND_MODEL=gpt-5.6-luna WEARER="${WEARER:-Saint Louis, Saint, Bootoshi}" sv run OPENAI_API_KEY,EXA_API_KEY -- npm run dev
    up_one metro    "$KIT" bunx expo start --dev-client --offline --clear
    up_one phone    "$ROOT" env LAN_IP="$(ipconfig getifaddr en0)" bun web/scripts/lan-https.ts
    echo
    echo "phone relay URL:  ws://$IP:8790/glasses"
    echo "phone Mac URL:    https://$IP:3443/"
  ...
  status)
    for n in mediamtx relay vision web metro phone; do
    ...
    echo "phone relay URL: ws://$IP:8790/glasses"
    echo "phone Mac URL:   https://$IP:3443/  (self-signed; accept once)"
    ;;
