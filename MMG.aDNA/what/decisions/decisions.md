...
| D24 | Lisa's `sidecar.ts` (HTTP client → Python `:7777`) coexists on main, dormant (nothing imports it). Which of {sqlite, sidecar, json} `index.ts` defaults to is an open team call; docs branch `memory-docs-sync` stays unmerged until decided | 13:10 |
