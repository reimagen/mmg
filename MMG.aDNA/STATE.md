...
- **Whole room imported 15:25 (D35): roster = 83 people, 79 with real context.** The event Networking page lists all 77 attendees with company and role behind a login — scraped its four pages and imported. Nearly anyone in the room now gets a card off a first name ("nice to meet you, Dhravya" → Supermemory, Founder). Corrections it surfaced: Seth is **Seth Tamrowski** (Oxen AI, GTM), and Jake's own profile reads "AILedger — Co-founder".
- **Wrong-person fact incident 15:55 (D36):** Seth's card carried a Claude Code grammar-parser bug from a GitHub issue filed by a *different* Seth. Three stacked causes — the seeded name was my guess ("Seth Tam"; the attendee list says **Seth Tamrowski, Oxen AI — GTM**), a full name was allowed to skip corroboration, and the match was a substring so "Seth Tam" hit "Seth Tams". Fixed: research needs the employer/project the person actually said for **everyone**, names match on a word boundary, seed corrected. Live job now reports the honest skip.
- **Backend made visible (D37):** [[what/mmg/docs/BACKEND]] is the one-page front door — turn it on, the single `/api/runtime`, what happens on a turn, the features people would not otherwise find, lanes, scripts. Linked from README/AGENTS/CLAUDE/SHIP. `npm run backend:check` prints the whole loop working in 20 s.
- **The roster ships in the repo (Jake 15:32):** `web/roster.json` is committed (83 people) and hydrates the SQLite roster table on first read, so a teammate only needs `git pull && npm run dev` — no scraping, no keys. Refresh both copies with `npm run roster:export` after any import. Names/orgs/one-liners only, no contact details.
- **The contact graph is NOT buried in MMG.aDNA (Jake 15:20):** `npm run roster:export` writes it to [[operations_jake.aDNA/what/context/people/roster_2026-09-12_openai_global_hackathon]] (markdown + JSON), committed there. The people outlast the build.
- **Real event roster imported 15:05 (D34):** the hackathon team page is login-only, so no crawler or Exa search can reach it — scraped it from the browser and posted the 16 team blurbs to the new `POST /api/roster/import`, which model-extracts the people. **Roster: 20 people, 16 with real context** (Callahan Stroud/Prairie Labs, Escobedo brothers/Locked In, Parkhurst + Akin/Aitherium, Stomo/bezeeai, Bhat/G-Unit, Miller/Otaku, Fernandez/PlayQuest, Pisipati/Renderwolf, Deciga/Meerkatt + our team + sponsor founders). Beat: "nice to meet you, Prashant" → "team Renderwolf — CEO of RenderWolf AI…" with no mid-handshake search. A bare first name matches only when exactly one person at the event has it.
- **Roster + seed made real 14:50 (D33):** the seeded facts were placeholders I wrote, so "I am Jake" recalled fiction — `npm run seed` now loads the actual team from [[who/team]] (Greg/Oxen.ai lead, Jake/AILedger, Lisa/reimagen.ai, Luis, Saint, Seth, Teddy, Eric), `--empty` for a cold ledger. Open-web attendee discovery returns randoms, so pre-flight is inverted: `web/roster-seed.json` holds known names, each researched against a disambiguating hint; no confident match = name-only, never guessed. Cayden Pierce + Alexander Israelov removed (Jake: garbage data). Roster is now in `GET /api/runtime`.
- **Pre-flight roster shipped 14:30 (main @4c2fc4b, D30–D32):** `npm run preflight` researches the event + sponsors with Exa, the model extracts real named people, and they land in a `roster` table that is deliberately NOT the ledger. It feeds the keeper's name-correction list and warms the first card — live: "I am Greg, my company is Oxen AI" → "Greg — Works at Oxen AI. Oxen.ai — CEO of Oxen.ai (event roster)." with the source link. Current roster: 9 (Oxen.ai, MentraOS, Exa, organisers). **Re-run before the demo, never during.**
- **Employer extraction is the model's job (D30):** keeper returns `org`, persisted on `Person.org`, and research searches on it; the regex is a floor only.
- **No recognizer vocabulary in GPT Live (D32):** the session rejects `session.audio.input`. Remaining upstream route is a parallel biased transcription pass (Plaud/Wispr shape) — written up in HANDOFF, not built.
- **Misheard names (Jake 14:02, D29):** downstream fixed — a correction renames in place and keeps the old spelling as an alias, so "Stan" still recalls Sam; corrections and spelled-out names are detected. Upstream — Chrome's auto gain control was riding a +20 dB mic; `browser.ts` now pins AEC+NS on, AGC off, mono 48 kHz. Remaining lever (Lisa's): no input-transcription config on the Live session, so the recognizer can't be biased toward names already in memory.
- **Projector `web/public/screen.html` is a bundled design export with no `fetch`** — its "MEMORY DOWN / Memory API unreachable / MEMORY EMPTY" are static copy, not live readings. Memory was up with 6 people throughout. Wiring it needs `GET /api/runtime` + `GET /api/memory/people`. Note `health.memory` has had no writer since the Python sidecar retired; it sits at its default `"ok"` and is not a liveness signal.
...
- 15:20 closed (pre-demo verification): [[how/sessions/session_2026-09-12_1330_detection_research_roster]] — caught and fixed a roster first-name collision (Prashant/Robert/Seth listed twice); tsc 0, memory:check OK, backend:check 0, both demo beats verified; reseeded clean, server up in tmux `mmg`.
